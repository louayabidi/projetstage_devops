...
const OwnerCalendar = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        // Set default selected booking to the first one, if available
  ...
  const getEventColor = (status) => {
    switch (status) {
      case 'confirmed': return '#34d399'; // Emerald green
  ...
  if (error) return (
    <div className="text-center text-red-500 p-4 bg-red-100 rounded-lg m-4">{error}</div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-white p-6">
      <div className="max-w-7xl mx-auto">
        <div className="bg-gradient-to-r from-blue-600 to-teal-500 text-white p-6 rounded-t-lg shadow-lg">
          <h2 className="text-3xl font-bold">⛵ My Boat Bookings Calendar</h2>
          <p className="mt-2 text-lg">Manage your boat bookings with ease!</p>
          <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:space-x-4">
            <select
              className="p-2 border rounded-lg text-gray-800 bg-white focus:ring-2 focus:ring-blue-500 mb-2 sm:mb-0"
        ...
        <div className="bg-white p-4 rounded-b-lg shadow-lg">
          <div className="flex justify-between items-center mb-4">
            <div className="flex space-x-4">
              <span className="flex items-center"><span className="w-4 h-4 rounded-full bg-emerald-400 mr-2"></span>Confirmed</span>
              <span className="flex items-center"><span className="w-4 h-4 rounded-full bg-amber-400 mr-2"></span>Offered</span>
              <span className="flex items-center"><span className="w-4 h-4 rounded-full bg-blue-500 mr-2"></span>Pending</span>
              <span className="flex items-center"><span className="w-4 h-4 rounded-full bg-red-500 mr-2"></span>Canceled</span>
            </div>
            <select
              ...
              components={{
                event: ({ event }) => (
                  <Tooltip
                    title={
                      <div>
                        <strong>Boat:</strong> {event.resource.boat.name}<br />
                        <strong>Passenger:</strong> {event.resource.passenger.firstName} {event.resource.passenger.lastName}<br />
                        <strong>Persons:</strong> {event.resource.numberOfPersons}<br />
                        <strong>Status:</strong> {event.resource.status.toUpperCase()}<br />
                        <strong>From:</strong> {moment(event.start).format('MMM D, YYYY h:mm A')}<br />
